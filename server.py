            ...
            try:
                ...
                if "level" in data:
                    ...
                    if winner != '0':
                        ...
                        if winner != '0':
                            await player_ws.send(json.dumps({"winner": winner}))
                            await player_ws.send(json.dumps({"index": bot_move[0] * 7 + bot_move[1], "player": "O"}))
...
async def handle_connect4_player_mode(player1_ws, player2_ws):
    connect4 = Connect4()
    ...
    async def relay_moves(player_ws, opponent_ws):
        async for message in player_ws:
            try:
                ...
                print(connect4.board, flush=True)
    ...
    try:
        ...
        if waiting_player is None:
            ...
            if mode == "ai mode":
                ...
                await handle_connect4_ai_mode(waiting_player) 
                waiting_player = None
            ...
            if mode == "ai mode":
                ...
                await handle_connect4_player_mode(player1_ws, player2_ws)
